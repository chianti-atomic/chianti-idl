...
IF photoexcitation THEN $
  input =CREATE_STRUCT(input, 'dilute', dilute, 'radtemp', radtemp)
         ...
         IF NOT keyword_set(noprot) THEN BEGIN
           ...
           pe_rat=proton_dens(alog10(temp))

           inputs = create_struct(input, 'pe_ratio',pe_rat) 
...
;
; n_lev can be an output or input for pop_solver. We're using
; it as an output here, so if np is already defined, then we have to
; delete it with the temporary command.
;
         IF n_elements(np) NE 0 THEN junk=temporary(np)
         pop_solver,inputs,temp,dens,pops,n_lev=np,/pressure,frac_cutoff=frac_cutoff
         

;;          IF n_elements(density) EQ 1 THEN BEGIN
;;            junk=temporary(np)
;;            help,temp,dens
;;            pop_solver,inputs,temp,dens,pop,n_lev=np
;;            help,pop
;;            pops=reform(pop)
;;          ENDIF ELSE BEGIN
;;            junk=temporary(np)
;;            pop_solver,inputs,temp,dens,pops,n_lev=np,/pressure
;; ;           FOR i=0,nt-1 DO pops[i,*]=pop[i,i,*]
;;          ENDELSE 
