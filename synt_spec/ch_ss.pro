...
;---------------------------------------
   ...
   event.id EQ state.inst_butt: BEGIN
      str=['Instrumental broadening', $
           ' ', $
           'Setting this to a non-zero value broadens each of the spectral', $
           'lines with a Gaussian of the specified FWHM (in Angstroms),', $
           'so mimicking the effects of instrumental broadening.', $
           ' ', $
           'WARNING:  make sure that the FWHM value (if not set to zero) is larger than ', $
           ' the bin size']
      result = DIALOG_MESSAGE(str,/info)
   END
...
wminw1 = widget_text(sss1,/edit,xsize=9,ysize=1,uvalue='MINW1',$
                     value=strtrim(string(format='(f11.4)',xrange(0)),2), font=font)
...
wmaxw1  = widget_text(sss2,/edit,xsize=9,ysize=1,uvalue='MAXW1',$
                      value=strtrim(string(format='(f11.4)',xrange(1)),2), font=font)
...
ioneq_show=WIDGET_TEXT(dummy, value='')
