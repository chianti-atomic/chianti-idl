...
PRO POP_SOLVER, input, T, XNE, POP, N_LEVELS=N_LEVELS, data_str=data_str, $
                ...
                no_calc=no_calc, verbose=verbose
...
; KEYWORDS
;
;       PRESSURE    Normally populations are computed for all
;                   temperatures and densities, leading to a final
;                   array with dimensions (NT,ND,NL). If /PRESSURE is
;                   set, then the final output array will have
;                   dimensions (NT,NL), i.e., rate(i) will be computed
;                   using temp(i) and dens(i). This gives a
;                   significant speed advantage for ch_synthetic and
;                   other applications.
;
;       NO_CALC     For some routines that call pop_solver,
;                   it's necessary to make an initial call to
;                   pop_solver simply to find the number of levels in
;                   the model (emiss_calc is an example). The keyword
;                   /NO_CALC stops pop_solver immediately after
;                   N_LEVELS is calculated so that the full population
...
;
;       v.31  28-Jun-2017, Peter Young
;            Now allow the input structure to have the ioneq_file and
;            abund_file tags (see the routine ch_setup_ion). Modified
;            how pe_ratio is computed if pe_ratio is not a tag in the
;            input structure.
;
; VERSION     : 31  28-Jun-2017
;
;-


; ------
; The input structure has a set of "core" tags and a set of "optional"
...
;
; PRY: 28-Jun-2017
; I recommend that users no longer put pe_ratio in the input
; structure, but instead define the abund_file and ioneq_file tags so
; that pop_solver can compute the pe_ratio.
;
...
IF tag_exist(input, 'abund_file') THEN abund_file=input.abund_file
IF tag_exist(input, 'ioneq_file') THEN ioneq_file=input.ioneq_file
...
IF n_elements(SUM_MWL_COEFFS) NE 0 AND keyword_set(pressure) THEN BEGIN
  print,'%POP_SOLVER:  the keywords SUM_MWL_COEFFS and PRESSURE are not compatible with each other.'
  print,'              Returning...'
  return
ENDIF 
...
IF keyword_set(no_calc) THEN return
...
;
; Create the AA and AAT (i.e., AA-transpose) arrays from the A_VALUE input.
;
...
IF nj NE 0 THEN BEGIN
  print,'%POP_SOLVER: ['+trim(gname)+'] - '+trim(nj)+' A-values have been assigned to inverse transitions. These have been flipped.'
  k=i[j]
 ;
...
IF n_tags(prot_struc) NE 0 THEN BEGIN
 ;
 ; PRY, 28-Jun-2017
 ; The old behaviour was for the user to compute pe_ratio separately
 ; and then pass it to pop_solver through the input
 ; structure. I now allow for the input structure to contain the
 ; ioneq_file and abund_file tags, so that pop_solver can directly
 ; compute pe_ratio. If the user has created the input structure with
 ; ch_setup_ion, then ioneq_file will always be defined. abund_file
 ; may not be defined, but then proton_dens will just pick up the
 ; default file (!abund_file).
 ; The previous code is commented out below.
 ;
  IF n_elements(pe_ratio) EQ 0 THEN BEGIN
...
; EL MODIFICATIONS END HERE
  ...
  ENDIF ELSE BEGIN
    FOR j=0,nt-1 DO BEGIN
      qqx=qq[*,*,j]
      pp=matrix_solver(xne[j],aa,qqx,ppr[*,*,j],aax,c=c, $
                       frac_cutoff=frac_cutoff,verbose=verbose)
      IF status GT 0 and iz-ion ne 1 THEN BEGIN ; Includes rec/ion corrections if ion is not He-like
; EL MODIFICATIONS END HERE
        pp=correct_pops(pp,t[j],xne[j],input.ionrec,c, $
                        rrate=rrate,crate=crate,correction=correction, $
                        frac_low=frac_low, frac_high=frac_high)
      ENDIF
      pop[j,*]=pp
...
ENDIF ELSE BEGIN          ; non-maxwellians
  ...
  ppx=total(ppr,3)
  FOR i=0,nxne-1 DO BEGIN
    pp=matrix_solver(xne[i],aa,qqx,ppx,aax,c=c,frac_cutoff=frac_cutoff, $
                    verbose=verbose)
    pop[0,i,*]=pp
  ENDFOR
ENDELSE
