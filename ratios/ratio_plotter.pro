...
;             transitions. 
;
;       V.43, 8-Dec-2011, Peter Young
;             Modified how the widget is scaled for small screens.
;
;       V.44, 17-Aug-2015, Peter Young
;             Changed 'lambda' to 'lmbda' as IDL have introduced a new
;             function called lambda.
;
; VERSION     :   43, 8-Dec-2011
;
;-
...
IF ypoint LE yrange[1] AND ypoint GE yrange[0] THEN BEGIN 
     ...
     ;
      lmbda=emiss(index(i-1)).lambda
      CASE 1 OF
        lmbda LT 50.: wvl_label=trim(string(format='(f12.4)',lmbda))
        lmbda GT 10000.: wvl_label=trim(string(format='(f12.1)',lmbda))
        ELSE: wvl_label=trim(string(format='(f12.3)',lmbda))
      ENDCASE
     ;
...
;
; The following check's the user's screen size and introduces widget
; scroll bars if it thinks the screen is too small.
; I modified this on 8-Dec-2011 so that it works on my Mac (screen
; 1600x900). PRY.
;
device, get_screen_size = sz
IF sz[0] LT 1200 OR sz[1] LT 1000 THEN BEGIN
  x_scroll=sz[0]*0.8
  y_scroll=sz[1]*0.8
ENDIF ELSE BEGIN
  x_scroll=0
