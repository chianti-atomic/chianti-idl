...
;              radtemp:radt, ionrec:ionrec}
   ;
    self->populate, 10.^temp[j],10.^dens[j], radfunc=radfunc
    pop=self->getPopulation()
   ;
    emiss.em[j]=mult_factor*reform(pop.population[emiss.level2-1])
  ENDFOR
ENDIF ELSE BEGIN
...
emiss=emiss[ind]
topstr = {temperature:temp, ion_name:species, version:str1, emstr:emiss}
