...
; CALLING SEQUENCE:
;
;       DENSITY_RATIOS_OBJ,Ion,Wmin,Wmax,Dmin,Dmax,Density,Ratio,Description
;
;
; INPUTS:
;
...
; CALLS
;
;       ION2SPECTROSCOPIC, CONVERTNAME, READ_IONEQ,
;       CH_XMENU_SEL
;		CHIANTI_ION object
;
; EXAMPLE:
...
;
;		V.15, 17-Jul-2012 Ken Dere
;			Now calls the chianti_ion object to get emissivities - replacing emiss_calc

; VERSION     :   14, 6-Feb-12
;
;-
pro density_ratios_obj,ions,wmin,wmax,denmin,denmax,density,ratio,description, $
...
print, ' doing ion = ',ions
this_ion = obj_new('chianti_ion',ions)
this_ion->populate,tmax,density
...
this_ion->emiss_calc
emiss = this_ion->getEmiss()
; em = emiss.emstr
obj_destroy,this_ion

; em=emiss_calc(iz,ion,diel=dielectronic,temp=alog10(tmax), $
;               dens=alog10(density), noprot=noprot, $
;               /quiet,rphot=rphot,radtemp=radtemp,no_de=no_de)

FOR i=0l,n_elements(emiss.emstr)-1 DO emiss.emstr[i].em = emiss.emstr[i].em/density


ind=where( (emiss.emstr.lambda GE wmin) AND (emiss.emstr.lambda LE wmax) )

list_int=emiss.emstr[ind].em

nlist=n_elements(ind)

; for il = 0,nlist-1 do begin
; 	print,il,emiss.emstr[il].lambda, max(emiss.emstr[il].em)
; endfor

; print, ' version = ',emiss.version

IF trim(emiss[0].version) EQ '' THEN version = '' ELSE $
  version = 'V. '+emiss[0].version+' '
...
; print,' em = ',emiss.emstr[3]

intmax=max(emiss.emstr[ind].em)
intmin=intmax/1d3

; print, ' intminmax = ', intmin,intmax
;
...
FOR ilist=0l,nlist-1 DO BEGIN
   IF max(emiss.emstr[ind[ilist]].em) EQ intmax THEN BEGIN
      th=2
      ref=ilist
   ENDIF ELSE BEGIN
      th=1
   ENDELSE
   oplot,density,reform(emiss.emstr[ind[ilist]].em),th=th
ENDFOR
th=1
...
FOR ilist=0l,nlist-1 DO BEGIN
   iden=rand1(ilist)
   xyouts,density(iden),emiss.emstr[ind[ilist]].em[0,iden], $
     string(ilist,'(i4)'), $
...
wvlstr=strtrim(string(emiss.emstr[ind[ref]].lambda,'(f10.4)'),2)
...
for ilist=0l,nlist-1 do begin
   oplot,density,reform(emiss.emstr[ind[ilist]].em)/reform(emiss.emstr[ind[ref]].em)
endfor
;
;
FOR ilist=0l,nlist-1 DO BEGIN
   iden=rand2(ilist)
   xyouts,density(iden),emiss.emstr[ind[ilist]].em[0,iden]/emiss.emstr[ind[ref]].em[0,iden], $
     string(ilist,'(i4)') $
...
FOR i=0l,nlist-1 DO BEGIN
   ...
   IF emiss.emstr[ind[i]].flag EQ -1 THEN text_add = ' *' ELSE text_add ='  '

   options[i]=string(i, format='(i6)')+ $
     string(emiss.emstr[ind[i]].lambda,format='(f10.4)')+text_add+angstrom+$
     '  '+string(emiss.emstr[ind[i]].em[0,nden-1],format='(e10.2)')+'  '+ $
     emiss.emstr[ind[i]].lvl1_desc+' - '+emiss.emstr[ind[i]].lvl2_desc
...
IF n_n GT 1 THEN numerator=total(reform(emiss.emstr[i_n].em),2) $
ELSE numerator=reform(emiss.emstr[i_n].em)
IF n_d GT 1 THEN denominator=total(reform(emiss.emstr[i_d].em),2) $
ELSE denominator=reform(emiss.emstr[i_d].em)
...
FOR i=0l,n_n-1 DO BEGIN
   description=description+strtrim(string(emiss.emstr[i_n[i]].lambda,'(f10.4)'),2)
...
FOR i=0l,n_d-1 DO BEGIN
   desc_ascii = desc_ascii+strtrim(string(emiss.emstr[i_d[i]].lambda,'(f10.4)'),2)
   description=description+strtrim(string(emiss.emstr[i_d[i]].lambda,'(f10.4)'),2)
