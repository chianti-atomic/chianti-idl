...
;
; OPTIONAL OUTPUTS:
;       Desiga: This is a string array containing the configuration
;               descriptors (3rd column of file) which are not part of
;               the standard output, but are useful when converting to
;               the new ELVLC format.
;       Spda:   The letter descriptor for the orbital angular
;               momentum. 
;
; EXAMPLE:
;
...
;
;       Ver.3, 31-Jul-2017, Peter Young
;          This is the original version of read_elvlc, prior to the
;          change of format of the ELVLC files. It was copied from the
;          v7.1.5 CHIANTI release, and has been renamed
;          read_elvlc_old. Made DESIG= and SPDA= optional outputs.
;
;-
pro read_elvlc_old,filename,l1a,term,confa,ssa,lla,jja,ecma,eryda,ecmtha,erydth,ref, $
                   desiga=desiga, spda=spda
;
;
;
if n_params(0) lt 10 then begin
   print,''
   print,'Use:  IDL> read_elvlc_old,filename,l1,term,conf,ss,ll,jj,ecm,eryd,ecmth,erydth,ref'
   print,'                          [ desig= ]'
   print,''
   return
endif
