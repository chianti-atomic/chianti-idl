...
;     effective collision strengths (upsilons). For more information
;     on the UPS file format, please check CHIANTI Technical Report
;     No. 13. 
;
; CATEGORY:
;     CHIANTI; read; format.
...
;
; EXAMPLE:
;     IDL> read_ups, 'mg_6.ups', upsstr
;
; MODIFICATION HISTORY:
;     Ver.1, 31-May-2013, Peter Young
;     Ver.2, 31-May-2013, Peter Young
;       - the arrays i1,i2,i3 are now long integers.
;     Ver.3, 27-Jan-2017, Peter Young
;       - made the -1 at the end of the file optional; added error
;         check and parameter check; updated header.
;-

IF n_params() LT 1 THEN BEGIN
  print,'Use:  IDL> read_ups, ups_file, ups_struc'
  return
ENDIF

chck=file_search(upsname,count=count)
IF count EQ 0 THEN BEGIN
  print,'%READ_UPS:  the input file does not exist. Returning...'
  return
ENDIF 
...
;
; The use of -1 at the end of the file is optional, so the lines below
; check for it and remove it if necessary.
;
k=where(trim(ref) NE '-1',nk)
ref=ref[k]
