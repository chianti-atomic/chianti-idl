...
IF n_params() LT 2 THEN BEGIN
  print,'Use:  IDL> wgfa_plot_comp, fname1, fname2 [, /encap, xrange=, file1_name=, file2_name='
  print,'                           xyout1= '
  return
ENDIF 
   ...
   ;
    ...
    ENDIF ELSE BEGIN 
      ...
      IF jj[0] NE -1 THEN BEGIN
        ...
        plots,aval_1[i],plot_rat,psym=psym
      ENDIF
      jj=where(ratio LT 0)
      IF jj[0] NE -1 THEN BEGIN
...
print,'Plot symbols:'
print,'   Square:   transition is the strongest from the upper level in both models'
print,'   Triangle: transition is the strongest from the upper level only in model 1'
print,'   Cross:    transition is the strongest from the upper level only in model 2'
print,'   Star:     regular transition'
