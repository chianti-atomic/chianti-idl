...
;                   repeating this step. Update (19-Jan-2018): this
;                   keyword is obsolete now, but maintained for
;                   backwards compatibility. Please specify the ioneq
;                   and abund files through the optional inputs listed
;                   above. 
;
...
;
; EXAMPLE:
;             > two_photon,1.e+6,,wvl,int
;             > two_photon,1.e+6,wvl,int,min_abund=3.e-5
;             > two_photon,1.e+6,wvl,int,edens=3e10
;
; PROGRAMMING NOTES
;
...
IF n_elements(abund_file) EQ 0 THEN BEGIN
  ...
  print,'% TWO_PHOTON: abund_file was not specified, using default option...'
  print,'              '+trim(abund_file)
...
if keyword_set(sngl_ion) then begin
   mlist=sngl_ion
endif else read_masterlist,mname,mlist
;
...
;
      if abundtest then begin
         ...
         if ioneqtest then BEGIN
                                ;
           input=ch_setup_ion(gname,ioneq_file=ioneq_file,abund_file=abund_file)
           ecm=input.ecm
...
;            setup_ion,gname,-1,-1,wvltst,lvl1,lvl2,wvl1,gf1,a_value1
           ...
           ;
           ; PRY, 18-Jan-2018
           ; This is the index of the upper level of the two-photon
           ; transition. Note that the distinction between
           ; dielectronic and non-dielectronic ions was not made prior
           ; to this change.
           ; I do a check to make sure this is actually the upper
           ; level of the transition.
           ;
            IF dielectronic EQ 1 THEN pop_idx=heseq_lvl2d[iz-1]-1 ELSE pop_idx=heseq_lvl2[iz-1]-1
            chck_wvl=input.wvl[0,pop_idx]
            chck_aval=input.a_value[0,pop_idx]
            IF chck_wvl NE 0. OR chck_aval EQ 0. THEN BEGIN
              print,'% TWO_PHOTON: WARNING!!'
              PRINT,'              The He-sequence two-photon transition for '+trim(gname)+' appears to be incorrect.'
              print,'              Please contact the CHIANTI team.'
              print,chck_wvl,chck_aval
            ENDIF 
